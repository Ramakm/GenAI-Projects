...
async def lifespan(app: FastAPI):
    logger.info("Starting Regulatory Impact Analysis Agent backend...")
    try:
    ...
    yield
    logger.info("Shutting down Regulatory Impact Analysis Agent.")


app = FastAPI(
    title="Regulatory Impact Analysis Agent",
    description="5-node LangGraph pipeline: Document Parser → Clause Extractor → Industry Classifier → Impact Assessor → Action Plan Generator",
    version="1.0.0",
    lifespan=lifespan,
)
